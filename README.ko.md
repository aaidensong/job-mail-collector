...
3. 같은 대화에서 ChatGPT와 구직 조건 및 경력에 대해 이야기합니다.
...
> Senior Product Designer를 주로 찾고 있는데, 실제 업무가 제 경험과 맞으면 UX/UI나 Lead 역할도 같이 보고 싶어요. B2C 제품 디자인 경력은 약 9년이고 디자인 시스템과 Growth 쪽 경험도 많습니다.
...
- 필요한 경우 질문에 `필수` 또는 `선택`을 표시하지만, 정해진 형식으로 답할 필요는 없습니다.
- 생소한 개념은 쉬운 말로 설명합니다.
- 이미 답변한 내용은 다시 묻지 않습니다.
- 추천 결과를 실제로 바꿀 수 있는 정보가 부족할 때만 추가 질문을 합니다.
...
반대로 직무명은 비슷해 보여도 실제 요구 범위가 사용자의 경험과 크게 다르면 낮은 적합도로 판단할 수 있습니다.

공고를 판단할 때는 다음을 함께 봅니다.

- 실제 업무와 역할 범위
- 요구 경력과 오너십 수준
- 리더십과 People Management 요구 여부
- 제품과 산업 경험
- 관련 스킬, 성과, 경력 근거
- 지역과 근무 형태 조건
- 고용 형태, 취업 자격, 스폰서십, 급여 조건

따라서 사용자는 `Senior 외에 Staff도 볼 것인지`, `Lead도 볼 것인지`를 일일이 미리 정하기보다, 정말 원하지 않는 역할이나 조건만 명확히 알려주면 됩니다.

People Management와 Leadership도 별도로 봅니다. 프로젝트 리드, 멘토링, 디자인 방향 설정, 협업 주도 경험만으로 사람을 직접 관리한 경험이 있다고 판단하지 않습니다.

자세한 매칭 기준은 `docs/matching-rules.md`에서 확인할 수 있습니다.
...
5. ChatGPT가 실제 경력과 구직 방향을 먼저 파악합니다.
...
5. 명확한 비적합 공고와 중복 공고를 제거합니다.
6. 남은 공고를 사용자의 실제 경력 근거와 조건으로 비교합니다.
7. 직무명이 다르더라도 실제 업무 범위가 맞으면 유사 또는 상위 직급 공고도 검토합니다.
8. Google Drive 쓰기 권한이 허용되면 적합한 새 공고를 Tracker Sheet에 기록합니다.
9. 예약 작업 결과에서 가장 적합한 공고와 지원 링크를 반환합니다.
10. 명확한 지원 확인 메일과 회사 또는 리크루터 회신을 확인합니다.
11. 근거가 명확하면 Tracker 상태 필드를 자동으로 업데이트합니다.
12. 사람이 확인해야 하는 사항은 별도로 보고합니다.
...
- 정해진 형식이 아닌 자유로운 자연어 답변
- 조건보다 실제 경력 근거를 먼저 수집하는 온보딩
- 직무명과 직급보다 실제 역할 적합도를 우선하는 매칭
- 명시적인 하드 제외 조건 처리
- 비공개 커리어 프로필 생성
...
- **설문보다 대화.** 사용자는 여러 생각을 한 답변에 자연스럽게 섞어서 말할 수 있습니다.
- **조건보다 경력 근거를 먼저 수집.** 실제로 무엇을 해왔는지 이해한 뒤 필요한 제한만 확인합니다.
- **한 번에 질문 하나.** 여러 질문을 한꺼번에 던지지 않습니다.
- **한 번 답한 것은 다시 묻지 않음.** 하나의 답변에서 여러 내부 필드를 채울 수 있습니다.
- **직무명보다 실제 적합도.** 주요 직무와 직급은 기준점이지 자동 허용 목록이 아닙니다.
- **허용 목록보다 제외 조건을 확인.** 지원 가능성이 있는 역할은 열어두고 사용자가 명확히 원하지 않는 것만 닫습니다.
...
이번 변경은 온보딩 방식과 매칭 해석을 개선하지만 YAML 키와 Sheet 스키마는 바꾸지 않으므로 버전은 그대로 유지합니다.
