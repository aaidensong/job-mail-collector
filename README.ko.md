...
사용자와 GPT의 전체 역할 구분은 `docs/user-flow.md`에서 확인할 수 있습니다.
...
4. ChatGPT가 같은 대화에서 프로필과 설정에 필요한 질문을 합니다.
5. 사용자가 같은 대화에서 질문에 답합니다.
6. ChatGPT가 연결된 Google Drive에 개인 커리어 프로필과 Google Sheet를 생성합니다.
7. ChatGPT가 사용자가 선택한 시간에 반복 실행되는 예약 작업을 생성합니다.
8. 설정이 완료되기 전에 ChatGPT가 검증 테스트를 실행합니다.

설정이 끝난 뒤 생성된 프로필이나 트래커 파일을 사용자가 직접 옮길 필요는 없습니다. ChatGPT가 연결된 Drive에 저장하고 이후에도 해당 파일을 참조합니다.
...
- **사용자와 GPT의 역할을 명확하게 구분.** 사용자는 앱 연결, 질문 답변, 실제 지원을 담당하고 GPT는 수집, 매칭, 추적, 모니터링을 담당합니다.
...
`Config.config_version`은 Sheet 및 자동화 호환성을 추적합니다. 비공개 프로필은 YAML front matter의 `profile_version`을 별도로 사용합니다.
